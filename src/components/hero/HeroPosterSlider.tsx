...
export function HeroPosterSlider({ onSlideChange, className }: HeroPosterSliderProps) {
  ...
  // Autoplay — stable interval that checks pause refs on each tick
  useEffect(() => {
    if (prefersReducedMotion) return;

    const timer = setInterval(() => {
      if (isPausedRef.current || isTabHiddenRef.current || prefersReducedMotionRef.current) {
        return;
      }
      nextSlide();
    }, AUTOPLAY_INTERVAL);
  ...
  }, [prefersReducedMotion, nextSlide]);
      ...
      >
        {HERO_SLIDES.map((slide, index) => (
          <div
            ...
            className="relative w-full h-full flex-shrink-0 bg-slots-black overflow-hidden"
          >
            <Image
              src={slide.image}
              alt={slide.alt}
            ...
            />
            <div className="absolute inset-0 bg-gradient-to-r from-slots-black/85 via-slots-black/50 to-slots-black/20 pointer-events-none" />
            <div className="absolute inset-0 bg-gradient-to-t from-slots-black/80 via-transparent to-slots-black/30 pointer-events-none" />
          </div>
        ))}
      </div>

      {/* Numbered Indicators Bar at Bottom — Desktop & Tablet Only (shifted upward) */}
      <div className="absolute bottom-6 md:bottom-16 lg:bottom-20 left-4 right-4 md:left-8 md:right-8 z-30 hidden md:flex items-center justify-end gap-3 md:gap-4 pointer-events-auto">
